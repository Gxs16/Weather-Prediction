...
X=np.loadtxt('X_MAX.csv',delimiter=',')
y = np.loadtxt('y_max.csv',delimiter=',')
loss_y = np.isnan(y[6:])
y = y [6:]
loss_y = loss_y
use_y = loss_y ==0
new_y = y[use_y]
new_X=X[use_y]
y_true = np.loadtxt('y_true_max.csv',delimiter=',')
y_day_average = np.loadtxt('y_day_average_max.csv',delimiter=',')
#linear_model
...
np.savetxt('Linear\\y_predict_MAX.csv',y_r,delimiter=',')
