...
raw = np.loadtxt('Data_va\\temp.csv',delimiter=',')
origin = raw[:2922,:]
...
#day_average = np.loadtxt('Use\\X_dayave.txt',delimiter=',')
for i in range(828):
    origin_valid[i]-=day_average[i%365]
origin_valid[loss_valid] = 0
origin_valid /= origin_std
X_valid = pca.transform(origin_valid)
np.savetxt('X_temp.csv',X,delimiter=',')
np.savetxt('X_valid_temp.csv',X_valid,delimiter=',')